...
class ModelPerformanceTracker:
    ...
    def _load_performance_data(self) -> Dict:
        ...
        return {
            # Core 3 models - optimized for stability
...
class MultiLLMManager:
    ...
    def __init__(self):
        ...
        logger.info(f"Multi-key Groq system initialized with {len(self.groq_api_keys)} API keys")
        ...
        # Model definitions - Core 3 models optimized for stability
        self.models = {
            # Core 3 Groq Models - Most reliable and stable
            "versatile": {
        ...
        # Task-specific model assignments (Core 3 models only)
        self.task_models = {
            "strategy_generation": [
                "versatile", "analytical", "diverse"
            ],
            "evaluation": [
    ...
    def _test_model_availability(self):
        ...
        # Simplified system - Groq only
        print(f"🚀 Core 3 system: Using {len(available)} stable Groq models")
    ...
    def generate_with_model(self, prompt: str, model_key: str, timeout: float = 30.0, **kwargs) -> Tuple[str, Dict]:
        ...
        if provider == "groq":
            return self._generate_with_groq(prompt, model_key, model_name, params, timeout)
        else:
            raise Exception(f"Unsupported provider: {provider}. Only Groq models are supported in simplified mode.")
    
    def _generate_with_groq(self, prompt: str, model_key: str, model_name: str, params: Dict, timeout: float = 10.0) -> Tuple[str, Dict]:
        """Generate content using Groq API with timeout protection and logging"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(make_groq_call)
                    ...
                    try:
                        ...
                        if elapsed > 5.0:  # Log slow calls
                            self._log_timeout("slow_call", model_key, elapsed, f"Slow API call: {elapsed:.2f}s")
                        
                    except concurrent.futures.TimeoutError:
                        elapsed = time.time() - start_time
                        self._log_timeout("timeout", model_key, elapsed, f"API call timed out after {timeout}s")
                        raise TimeoutError(f"Groq API call timed out after {timeout}s")
                ...
                if isinstance(e, TimeoutError):
                    self._log_timeout("timeout", model_key, elapsed, str(e))
                elif is_rate_limit:
                    self._log_timeout("rate_limit", model_key, elapsed, f"Rate limit hit: {str(e)[:100]}")
                else:
                    self._log_timeout("error", model_key, elapsed, f"API error: {str(e)[:100]}")
                
                logger.error(f"Error with Groq model {model_key} (attempt {attempt + 1}/{max_retries}): {e}")
                
                # For rate limits, try switching API key
                if is_rate_limit:
                    logger.warning(f"⚠️ Rate limit hit for {model_key} with key {self.current_key_index + 1}")
                    
                    # Try switching to next API key
                    if self._rotate_api_key():
                        logger.info(f"🔄 Switched to API key {self.current_key_index + 1}, retrying...")
                        # Reset attempt counter to try with new key
                        attempt = -1  # Will be incremented to 0 in next iteration
                        continue
                    else:
                        logger.warning(f"⚠️ All API keys exhausted for {model_key}")
                        # Mark model as temporarily unavailable
                        self.available_models[model_key] = False
                        raise e
    ...
    def _rotate_api_key(self) -> bool:
        """Rotate to next available API key. Returns True if successful, False if all exhausted."""
        if len(self.groq_api_keys) <= 1:
            return False  # No other keys available
        
        # Try next key
        original_index = self.current_key_index
        for _ in range(len(self.groq_api_keys) - 1):
            self.current_key_index = (self.current_key_index + 1) % len(self.groq_api_keys)
            new_key = self.groq_api_keys[self.current_key_index]
            
            try:
                # Test new key with a minimal call
                self.groq_client = Groq(api_key=new_key)
                # Don't actually make a call here to avoid wasting tokens
                return True
            except Exception as e:
                logger.warning(f"API key {self.current_key_index + 1} failed: {e}")
                continue
        
        # All keys failed, revert to original
        self.current_key_index = original_index
        self.groq_client = Groq(api_key=self.groq_api_keys[original_index])
        return False
    
    def _log_timeout(self, event_type: str, model_key: str, elapsed: float, message: str):
        """Log timeout and performance events"""
        import os
        from datetime import datetime
        ...
        try:
            ...
            # Append to timeout log
            with open("logs/timeouts.log", "a") as f:
                import json
                f.write(json.dumps(timeout_entry) + "\n")
                
        except Exception as e:
            logger.error(f"Failed to log timeout event: {e}")
    ...
    def _find_fallback_model(self, preferred_model: str = None) -> str:
        ...
        # Priority order for fallbacks (Core 3 models only)
        fallback_priority = [
            "versatile", "diverse", "analytical"
        ]
